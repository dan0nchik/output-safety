...
class PIIDetectorRepository(IMLServiceRepository):
    ...
    PHONE_WORDS = [
        'номер телефона', 'номера телефона', 'номеру телефона',
        'номером телефона', 'номере телефона'
    ]
    PASSPORT_WORDS = [
        'паспорт', 'паспортные данные', 'паспортных данных',
        'паспортными данными', 'серия', 'номер'
    ]
    # Регулярки
    ...
    _ner_model = build_model(configs.ner.ner_ontonotes_bert_mult, download=True, install=True)

    def _find_phone(self, text: str) -> List[dict]:
        found = []
        for match in self.PHONE_REGEX.finditer(text):
            found.append({'type': 'PHONE', 'match': match.group(), 'span': match.span()})
        return found

    def _find_email(self, text: str) -> List[dict]:
        return [{'type': 'EMAIL', 'match': m.group(), 'span': m.span()} for m in self.EMAIL_REGEX.finditer(text)]
    ...
    def _find_passport(self, text: str) -> List[dict]:
        ...
        if any(word in text.lower() for word in self.PASSPORT_WORDS):
            for match in self.PASSPORT_REGEX.finditer(text):
                found.append({'type': 'PASSPORT', 'match': match.group(), 'span': match.span()})
            for match in self.PASSPORT_SERIES_REGEX.finditer(text):
                found.append({'type': 'PASSPORT_SERIES', 'match': match.group(), 'span': match.span()})
            for match in self.PASSPORT_NUMBER_REGEX.finditer(text):
                found.append({'type': 'PASSPORT_NUMBER', 'match': match.group(), 'span': match.span()})
        return found
    ...
    def _find_fio(self, text: str) -> List[dict]:
        ...
        for idx, (token, tag) in enumerate(zip(tokens, tags)):
            ...
            if is_b:
                ...
                if otchestvo_span:
                    found.append({'type': 'FIO_OTCHESTVO', 'match': otchestvo_span['match'], 'span': otchestvo_span['span']})
                    otchestvo_span = None
                current = [token]
                i_person_counter = 0
                # Позиция фамилии
                ...
                if i_person_counter == 2:
                    ...
                    otchestvo_span = {'type': 'FIO_OTCHESTVO', 'match': token, 'span': (otchestvo_start, otchestvo_end)}
            else:
                if otchestvo_span:
                    found.append({'type': 'FIO_OTCHESTVO', 'match': otchestvo_span['match'], 'span': otchestvo_span['span']})
                    otchestvo_span = None
